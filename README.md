...
**项目＝大号任务**（长期、持续、不定期迭代），但节奏不同：**只有「执行中／已完结」两态**（内部复用 `executing`/`done`）。建项目必须写清目标／说明（不能只有一个名字）；完结允许遗留未完成任务（自动留痕「完结时遗留 N 项」）；可重开续作；项目不挂起、不走计划闸。项目总览按此呈现：卡＝目标＋🔔待你处理＋最近动静（进度百分比对持续流是假指标，不设）。

**任务层主干四阶段**是一条线，**挂起是与它平行的中断字段**（不是阶段，是“原地举手”）：
...
| 项目两态 | 顶层任务只有 `executing↔done`（完结／重开）；建项目 `goal` 必填；不挂起、不可对项目提问；完结允许遗留（留痕不拦） |
| 完结即休眠 | 项目完结后：遗留任务的挂起不再进「待你处理」／IM 推送／找活面；遗留任务禁 推进/领取/提问/写计划/交产出（留言与删除放行——归档整理不算续作）；重开即全部恢复 |
...
| 方法与路径 | 作用 |
|---|---|
| `GET /api/projects` | 项目总览 `{active, closed}`（项目卡带目标、「待你处理」计数、最近动静） |
