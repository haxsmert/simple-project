...
// 项目=大号任务(2026-07-19 定调)后, 顶层任务走项目规则(两态) ——
// 任务机制的 fixture 一律挂在宿主项目下, 否则测的是项目分支不是任务分支
const host = (db: DB): string => createTask(db, { id: 'P-H', title: '宿主项目', state: 'executing', goal: '装任务用' }).id;
...
describe('handoff', () => {
  it('换手改变负责人/角色/状态, 并留下换手记录', () => {
    ...
    const t = createTask(db, {
      title: '搭建数据层', parentId: h, state: 'executing',
  ...
  it('拒绝非法状态流转', () => {
    ...
    const t = createTask(db, { title: 'x', parentId: h, state: 'executing', currentActor: 'exec', currentRole: 'executor' });
    expect(() => handoff(db, {
  ...
  it('原地改派不改变角色: 把等确认的任务转给 planner 角色被拒(矛盾位之源); 换决策者(同角色)放行', () => {
    ...
    const t = createTask(db, { title: 't', parentId: h, state: 'planning', hold: 'confirm', currentActor: 'admin', currentRole: 'decider', planMd: '- [ ] x' });
    ...
    // 未挂起的普通改派同样保角色
    const e = createTask(db, { title: 'e', parentId: h, state: 'executing', currentActor: 'p', currentRole: 'executor' });
  ...
  it('自批闸在机制层: 提交确认不能交给自己; 确认挂起中改派也不能转回提交人(MCP 绕不过)', () => {
    ...
    createActor(db, { id: 'admin', name: 'admin', type: 'human' });
    createActor(db, { id: 'admin2', name: 'admin2', type: 'human' });
    const t = createTask(db, { title: 't', parentId: h, state: 'planning', currentActor: 'p', currentRole: 'planner', planMd: '- [ ] x' });
    // 提交给自己批 → 拒
  ...
  it('自批闸守住批准方向: 提交人不能自己批准通过; 决策者批准/打回正常; agent 不能冒名驱动别人的任务', () => {
    ...
    const t2 = createTask(db, { title: 't2', parentId: h, state: 'planning', currentActor: 'e', currentRole: 'planner', planMd: '- [ ] y' });
    createActor(db, { id: 'admin2', name: 'admin2', type: 'human' });
  ...
  it('终态与建子守卫(对抗审计 P1/P2): 全等空转幂等不堆事件', () => {
    const db = openDb(':memory:');
    const h = host(db);
    createActor(db, { id: 'admin', name: 'admin', type: 'human' });
    createActor(db, { id: 'x', name: 'X', type: 'agent' });
    const t = createTask(db, { title: '在做', parentId: h, state: 'executing', currentActor: 'x', currentRole: 'executor' });
  ...
  it('有未完成子任务不得标记完成(硬闸, 任务层); 子全完成后放行; 进测试不拦', () => {
    const db = openDb(':memory:');
    const h = host(db);
    createActor(db, { id: 't', name: 'T', type: 'agent' });
    const parent = createTask(db, { title: '父', parentId: h, state: 'testing', currentActor: 't', currentRole: 'tester' });
    ...
    const p2 = createTask(db, { title: '父2', parentId: h, state: 'executing', currentActor: 't', currentRole: 'executor' });
  ...
  it('从待规划推进必须有计划: 两条路(直接开工/先过确认)都拦, 同态改派不拦, 写了计划就放行', () => {
    ...
    const h = host(db);
    createActor(db, { id: 'p', name: 'P', type: 'agent' });
    const bare = createTask(db, { title: '没计划', parentId: h, state: 'planning', currentActor: 'p', currentRole: 'planner' });
    ...
    // 空白字符不算计划
    const blank = createTask(db, { title: '空白计划', parentId: h, state: 'planning', currentActor: 'p', currentRole: 'planner', planMd: '  \n ' });
    expect(() => handoff(db, { taskId: blank.id, byActor: 'p', toActor: 'p', toRole: 'executor', toState: 'executing' }))
      .toThrow(/还没有计划/);
    const planned = createTask(db, { title: '有计划', parentId: h, state: 'planning', currentActor: 'p', currentRole: 'planner', planMd: '- [ ] 第一步' });
    expect(handoff(db, { taskId: planned.id, byActor: 'p', toActor: 'p', toRole: 'executor', toState: 'executing' }).state)
      .toBe('executing');
  });
});

// 项目 = 大号任务(2026-07-19 用户定调): 长期、持续、不定期迭代 —— 只有「执行中/已完结」两态
describe('handoff 项目规则', () => {
  it('完结允许遗留未完成任务并留痕(拍板: 长期流的搁置/转向是真实结局); 重开回到执行中', () => {
    const db = openDb(':memory:');
    createActor(db, { id: 'admin', name: 'admin', type: 'human' });
    const p = createTask(db, { id: 'R-1', title: '长期方向', state: 'executing', currentActor: 'admin', currentRole: 'planner', goal: '方向说明' });
    createTask(db, { title: '遗留甲', parentId: p.id, state: 'executing' });
    createTask(db, { title: '遗留乙', parentId: p.id, state: 'planning' });
    // 完结: 任务层"子未完不得 done"的硬闸对项目不适用, 改为事件如实留痕
    const closed = handoff(db, { taskId: p.id, byActor: 'admin', toActor: 'admin', toRole: 'planner', toState: 'done', note: '方向搁置' });
    expect(closed.state).toBe('done');
    const ev = listEvents(db, p.id).at(-1)!;
    expect(ev.body).toContain('方向搁置');
    expect(ev.body).toContain('完结时遗留 2 项未完成');
    expect(ev.body).toContain('遗留甲');
    // 重开(done→executing 项目特例; 普通任务 done 仍是终态)
    expect(handoff(db, { taskId: p.id, byActor: 'admin', toActor: 'admin', toRole: 'planner', toState: 'executing' }).state).toBe('executing');
  });

  it('两态之外的流转拒; 项目不挂起; 原地改派(换负责人)放行', () => {
    const db = openDb(':memory:');
    createActor(db, { id: 'admin', name: 'admin', type: 'human' });
    createActor(db, { id: 'admin2', name: 'admin2', type: 'human' });
    const p = createTask(db, { id: 'R-1', title: '方向', state: 'executing', currentActor: 'admin', currentRole: 'planner', goal: 'g' });
    expect(() => handoff(db, { taskId: p.id, byActor: 'admin', toActor: 'admin', toRole: 'planner', toState: 'testing' }))
      .toThrow(/两态/);
    expect(() => handoff(db, { taskId: p.id, byActor: 'admin', toActor: 'admin', toRole: 'decider', toHold: 'confirm' }))
      .toThrow(/项目不挂起/);
    // 换负责人(原地, 同角色)
    expect(handoff(db, { taskId: p.id, byActor: 'admin', toActor: 'admin2', toRole: 'planner' }).currentActor).toBe('admin2');
