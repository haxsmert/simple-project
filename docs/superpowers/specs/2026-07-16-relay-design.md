...
- 父子最小不变量(方案 B): **完成的任务不能有没完成的子** —— 进「完成」前直接子任务必须全完成(硬闸仅此一条); 进测试时子未完不拦, 界面如实提示。
...
### 9.1 项目=大号任务, 两态(2026-07-19 用户定调, 推翻"项目也套四阶段")

用户原话拆解: 项目就是**大号任务**——周期长、持续、**不定期迭代**; 所以任务该有的说明它一定要有(目标/说明必填, 不能是"一个名字和壳"); 但它的节奏不同: **只有「执行中/完结关闭」两个状态**; 进度条对项目没意义(持续追加任务的流, 百分比昨天 80% 今天 40%, 是假指标)。

| 规则 | 落点 |
|---|---|
| 两态 `executing↔done`(完结/重开) | `core/handoff` 项目分支; 复用既有枚举不加新值, UI 层叫「执行中/已完结」 |
| 建项目 `goal` 必填, 建即执行中 | `service.createTask` 根任务分支(三通道同拦); Web 新建表单双输入 |
| 完结允许遗留未完成任务, 自动留痕 | 任务层"done 父不得有未完成子"硬闸对项目豁免, 事件 body 记「完结时遗留 N 项: 标题(编码)…」 |
| 项目不挂起、不可对项目提问 | handoff 拒 toHold; `raiseClarification` 拒根任务 parent |
| 已完结项目的遗留任务不进找活面 | `allTasksBoard` 只聚合执行中项目 |
| 项目总览 = 项目层透镜 | `projectOverview(): {active, closed}`; 卡=目标+🔔待处理+最近动静(全树最新事件); 已完结折叠沉底; 无进度环 |
| 迁移 | 开库幂等归一: 根任务 planning/testing→executing, 根上挂起清除 |
